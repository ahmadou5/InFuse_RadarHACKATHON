    ...
    const fetchBalances = async () => {
        try {
            if(tokenId === 'solana') {
                ...
                if(!user) return
                const balance = await getSplTokenBalance(connection ,tokenInfo[0].address, user?.publicKey)
                console.log(balance)
                setUserBalance(balance)
            }
        } catch (error: unknown) {
    ...
    return(
        ...
        <div className=" bg-slate-50/0 mb-[20px] py-2 px-2 h-[80px] w-[100%] flex  ">
             <div className="bg-white/5 flex items-center ml-7 justify-center w-[70px] rounded-full h-[70px]">
                <img className="rounded-full w-[98%] h-[98%]" src={tokenId === 'solana' ? 'https://solana-wallet-orcin.vercel.app/assets/5426.png' : tokenInfo[0]?.logoUrl} />
             </div>
            <div className='ml-auto text-xl font-bold mt-8 mr-2'>
            <p>{`${userBalance} ${tokenId === 'solana' ? 'SOL' : tokenInfo[0]?.name}`}</p>
            </div>
        </div>
        </div>
    )
